...
class Contenedor{
    ...
    try{
        ...
        try{
            ...
            return {status:"success",message:"Producto cargado exitosamente al carrito!"}
        }catch{
            return {statis:"error",message:"No se pudo cargar al producto al carrito"} 
        }
    }catch{
        let fecha = new Date;
        let timestamp = fecha.getDate()+'/'+(fecha.getMonth()+1)+'/'+fecha.getFullYear();
        ...
        try{
            ...
            return {status:"error",message:"no se puedo cargar el producto"}
        }
    }
}
...
async newCarrito(){
    try{
        ...
        try{
            ...
            return {status:"success",message:"Nuevo carrito creado de id: "+carritoNuevo.id}
        }catch{
            return {statis:"error",message:"No se pudo crear carrito"} 
        }
    }catch{
        let fecha = new Date;
        let timestamp = fecha.getDate()+'/'+(fecha.getMonth()+1)+'/'+fecha.getFullYear();
...
async agregarProductosCarrito(idCarrito,productos){
    try {
        ...
        if(index === -1 ){
            ...
            await fs.promises.writeFile(carritoURL,JSON.stringify(carritoNuevo,null,2));
            return {status:"success", message:"Se agrego el producto al carrito exitosamente."}
        }
    } catch (error) {
        return {status:"error", message:"No se pudo agregar el producto correctamente."}
    }
}
...
async getById(idProducto){  
    ...
    async deleteById(id){
        try{
            ...
            if(id > products.length){
                return {status:"error", message:"No hay producto con el id detallado"}
            }
    ...
    async deleteAll(){
        try{
            ...
            return{status:"error",message:"Error al querer eliminar productos"}
        }
    }
